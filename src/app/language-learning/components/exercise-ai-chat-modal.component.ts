...
export interface ExerciseAiChatModalData {
  exercise: any;
}
...
@Component({
  selector: 'app-exercise-ai-chat-modal',
  standalone: true,
  ...
  template: `
    <h2 mat-dialog-title style="display: flex; align-items: center; gap: 8px; justify-content: space-between;">
      ...
      @if (messages().length > 0) {
        <button mat-icon-button (click)="clearChat()" title="Clear chat" style="margin-left: auto;">
    ...
    <mat-dialog-actions>
      <div style="display: flex; width: 100%; gap: 8px;">
        <mat-form-field appearance="outline" style="flex: 1;">
          <input matInput [(ngModel)]="userInputText" placeholder="Ask a question about this exercise..."
...
export class ExerciseAiChatModalComponent {
  ...
  constructor(
    public dialogRef: MatDialogRef<ExerciseAiChatModalComponent>,
    @Inject(MAT_DIALOG_DATA) public data: ExerciseAiChatModalData,
  ) {}
  ...
  sendMessage(): void {
    ...
    const context = ex ? `\nContext: The exercise is "${ex.practiceLanguageText || ex['practice_language_text']}" (${ex.nativeLanguageText || ex['native_language_text']}).` : '';
    const prompt = msgs.map(m => `${m.role}: ${m.content}`).join('\n') + `\nassistant: ${context}`;

    let accumulated = '';
    this.llmService.generateWithDeepseekStream(
      prompt,
      (chunk: string) => {
        accumulated += chunk;
